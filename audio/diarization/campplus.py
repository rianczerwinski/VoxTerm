...
"""CAM++ speaker embedding model (vendored from WeSpeaker).

Source: https://github.com/wenet-e2e/wespeaker
Adapted from: https://github.com/alibaba-damo-academy/3D-Speaker

Modifications for VoxTerm:
- Inlined TSTP pooling layer (removed wespeaker.models.pooling_layers dependency)
- Self-contained: only requires torch and torch.nn
...
# ── pooling (inlined from wespeaker/models/pooling_layers.py) ──
...
class CAMLayer(nn.Module):
    ...
    def seg_pooling(self, x, seg_len: int = 100, stype: str = 'avg'):
        if stype == 'avg':
            seg = F.avg_pool1d(x, kernel_size=seg_len, stride=seg_len,
                               ceil_mode=True)
        elif stype == 'max':
            seg = F.max_pool1d(x, kernel_size=seg_len, stride=seg_len,
                               ceil_mode=True)
        else:
            raise ValueError('Wrong segment pooling type.')
        shape = seg.shape
        seg = seg.unsqueeze(-1).expand(
            shape[0], shape[1], shape[2], seg_len
    ...
    def __init__(self, num_layers, in_channels, out_channels, bn_channels,
        ...
        for i in range(num_layers):
            layer = CAMDenseTDNNLayer(
                ...
                bias=bias, config_str=config_str)
...
class FCM(nn.Module):
    ...
    def __init__(self, block, num_blocks, m_channels=32, feat_dim=80):
        ...
        self.layer1 = self._make_layer(block, m_channels, num_blocks[0],
                                       stride=2)
        self.layer2 = self._make_layer(block, m_channels, num_blocks[1],
                                       stride=2)
...
class CAMPPlus(nn.Module):
    ...
    Input:  (batch, time, feat_dim) Fbank features
    Output: (batch, embed_dim) L2-ready speaker embeddings
    """

    def __init__(self, feat_dim=80, embed_dim=512, pooling_func='TSTP',
                 growth_rate=32, bn_size=4, init_channels=128,
                 config_str='batchnorm-relu'):
        super(CAMPPlus, self).__init__()

        self.head = FCM(block=BasicResBlock, num_blocks=[2, 2],
                        feat_dim=feat_dim)
        ...
        self.pool = _POOLING[pooling_func](in_dim=channels)
        self.pool_out_dim = self.pool.get_out_dim()
        self.xvector.add_module('stats', self.pool)
        self.xvector.add_module(
            'dense',
            DenseLayer(self.pool_out_dim, embed_dim,
